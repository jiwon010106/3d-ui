...
const VideoCarousel = () => {
  ...
  const videoContainelRef = useRef([]);
      ...
      let animationGsap = gsap.to(span[videoId], {
        onUpdate: () => {
          ...
          if (progress !== currentProgress) {
            ...
            gsap.to(videoContainelRef.current[videoId], {
  ...
  const handleProcess = (type, i) => {
    switch (type) {
      ...
      case "pause":
        ...
        break;
      case "video-reset":
        setVideo((pre) => ({ ...pre, isPlaying: false, videoId: 0 }));
        break;
      default:
        return video;
    }
  ...
  return (
    <div>
            ...
            <div className="video-carousel_container">
              <div className="w-full h-full flex-center rounded-2xl overflow-hidden bg-black">
                <video
                  ...
                  onLoadedMetadata={(e) => handleLoadeMetadata(e, idx)}
                  // className={`${
      ...
      <div className="relative flex-center mt-10">
        <div className="flex-center py-5 px-7 bg-gray-800 backdrop-blur-sm rounded-full">
          {videoRef.current.map((_, idx) => (
            <span
              key={idx}
              className="mx-2 w-3 h-3 bg-gray-200 rounded-full  relative"
              ref={(el) => (videoContainelRef.current[idx] = el)}
            >
              <span
